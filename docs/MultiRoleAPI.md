...
let projectId = "projectId_example" // String | Project ID.
let roleSlug = "roleSlug_example" // String | Role slug to apply the preset to (e.g. `customer`).
let multiRoleApplyRoleFeaturePresetRequest = multiRole_applyRoleFeaturePreset_request(preset: "preset_example") // MultiRoleApplyRoleFeaturePresetRequest | JSON body with `preset` set to `admin`, `user`, or `viewer`.
...
let projectId = "projectId_example" // String | Project ID whose multi-role matrix is returned.
...
let projectId = "projectId_example" // String | Project ID used to resolve role configuration.
let multiRoleSimulateAppPermissionsRequest = multiRole_simulateAppPermissions_request(role: "role_example", operationId: "operationId_example", method: "method_example", pathname: "pathname_example", path: "path_example") // MultiRoleSimulateAppPermissionsRequest | Role slug plus either OpenAPI `operationId` or HTTP `method` and `pathname` to evaluate.
