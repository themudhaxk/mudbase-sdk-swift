...
# **usersVerifyEmail**
```swift
    open class func usersVerifyEmail(usersVerifyEmailRequest: UsersVerifyEmailRequest, completion: @escaping (_ data: MessageResponse?, _ error: Error?) -> Void)
```
...
let usersVerifyEmailRequest = users_verifyEmail_request(token: "token_example", projectId: "projectId_example") // UsersVerifyEmailRequest | Verification token from the email link; optional projectId for project context.

// Verify email address (organization and project)
UsersAPI.usersVerifyEmail(usersVerifyEmailRequest: usersVerifyEmailRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
